  ...
  return (
    <div className="min-h-screen relative overflow-hidden bg-black">
      ...
      <div className="absolute inset-0">
        ...
        <div className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-gradient-radial from-indigo-500/8 via-pink-500/4 to-transparent rounded-full animate-pulse-slow-reverse blur-3xl"></div>
      </div>
                ...
                <form onSubmit={handleSubmit} className={`space-y-5 transition-all duration-1500 delay-500 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
                      ...
                      {focusedField === 'email' && (
                        <div className="absolute inset-0 overflow-hidden rounded-2xl pointer-events-none">
                          {[...Array(4)].map((_, i) => (
                            <div
                              key={i}
                              className="absolute w-1 h-1 bg-cyan-400 rounded-full animate-particle-float opacity-60"
                              style={{
                                left: `${20 + (i * 15)}%`,
                                top: `${30 + (i * 10)}%`,
                  ...
                  <div className="flex items-center justify-between text-xs pt-2">
                    <label className="flex items-center space-x-2 cursor-pointer group">
                      <div className="relative">
                        <input
                          ...
                          className="sr-only"
                        />
                        <div className={`w-4 h-4 border-2 border-white/20 rounded bg-black/30 backdrop-blur-xl transition-all duration-300 ${formData.rememberMe ? 'border-cyan-400 bg-gradient-to-br from-cyan-400 to-purple-500' : 'group-hover:border-white/40'}`}>
                          {formData.rememberMe && (
                    ...
                    <Link 
                      ...
                      className="text-cyan-400 hover:text-cyan-300 transition-colors duration-300 tracking-wide hover:underline"
                    >
                      Forget Password
                    </Link>
                  </div>
...
{/* Simple Transparent Neural Login Button */}
<button
  type="submit"
  disabled={isLoading}
